...
public class TowerDefenseAssignment2 extends Application {
	...
	Landscape[][] grid = new Landscape[gridColumns][gridRows];
	Group gridGroup = new Group(); // displays the grid

	Tower[] towerList = new Tower[10]; // Holds all stationary towers
	...
	public void start( Stage primaryStage ) {
		...
		for (int i = 0; i < towerList.length; i++) {
			pane.getChildren().add(towerList[i].imageView);
		}
		drawNPCs();
		for (int i = 0; i < enemyList.length; i++) {
			pane.getChildren().add(enemyList[i].imageView);
		}
		
		System.out.println(enemyList[9]);
		...
		for (int i = 0; i < enemyList.length; i++) {
			createPath( enemyList[i], enemyList[i].getX(), enemyList[i].getY() );
		}
	...
	private void createPath( Enemy e, int oldX, int oldY ) {
		Random r = new Random();	
		...
		// rotate transition
		RotateTransition rt = new RotateTransition( Duration.millis(1000), e.imageView );
		rt.setFromAngle(-5);
		rt.setByAngle(10);
